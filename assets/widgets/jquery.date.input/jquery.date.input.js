  ...
  DateInput = (function($) { // Localise the $ function
    ...
    DateInput.prototype = {
      build: function() {
        ...
        // jquery1.3?汾֮?󲻽??ʹ?userAgent???????ַ???ʶ?????????Ͱ汾?ţ????1.9?????Ƴ?$.brower
        ...
        if (uaMatch(ua).browser === 'msie' && uaMatch(ua).version < 7) {
          isIe6 = true;
        };
      ...
      selectMonth: function(date) {
        ...
        if (!this.currentMonth || !(this.currentMonth.getFullYear() == newMonth.getFullYear() &&
          ...
          $(".selectable_day", this.tbody).click(this.bindToObj(function(event) {
            var dateStr = $(event.target).attr("date");
            dateStr = dateStr.replace(/-/gm, _CONSTANT.SEPARATOR);
            this.changeInput(dateStr);
          }));
  ...
  */
    ...
    $.date_input = {
      initialize: function(opts) {
        $("input.j-date-input").date_input(opts);
      }
    };
